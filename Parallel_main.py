...
def main(args):
    ...
    modes  = args.modes     # 时间维度的截断数量
    Lmodes = args.LBOmodes  # NORM中LBO基的截断数量
    width  =  args.width
    ...
    y_dataIn = Train_Data['U_field']
    x_dataIn = Train_Data['U_initial']

    # y_dataIn = y_dataIn.transpose(0,2,1)
    print("要进行基分解的数据的维度：",y_dataIn.shape)
    print("***如果是求空间维度的基，请将数据调整为：num * Nt * Nx")
    print("***如果是求时间维度的基，请将数据调整为：num * Nx * Nt")
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            out  = torch.einsum('ntm,mx->ntx',out_weight,spatial_basis)
